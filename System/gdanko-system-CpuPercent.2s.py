...
def gather_cpu_info():
    try:
        ...
        list_t1_combined.append(get_time_stats_tuple(user=combined_t1.user, system=combined_t1.system, nice=combined_t1.nice, idle=combined_t1.idle))
        list_t2_combined.append(get_time_stats_tuple(user=combined_t2.user, system=combined_t2.system, nice=combined_t2.nice, idle=combined_t2.idle))
        ...
        for i, cpu_instance in enumerate(t1):
            list_t1_individual.append(get_time_stats_tuple(cpu=i, user=cpu_instance.user, system=cpu_instance.system, nice=cpu_instance.nice, idle=cpu_instance.idle))
        for i, cpu_instance in enumerate(last_per_cpu_times2):
            list_t2_individual.append(get_time_stats_tuple(cpu=i, user=cpu_instance.user, system=cpu_instance.system, nice=cpu_instance.nice, idle=cpu_instance.idle))
        for i in range(len(t1)):
            output_individual.append(calculate(list_t1_individual[i], list_t2_individual[i]))

        return output_combined, output_individual
